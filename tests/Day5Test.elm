...
suite =
    describe "Day5 Tests"
        [ describe "Day2 Examples"
            [ test "First Example" <|
                \_ -> parse example1 |> run Nothing [] 0 |> first |> equal (Array.fromList [ 3500, 9, 10, 70, 2, 3, 11, 0, 99, 30, 40, 50 ])
            , test "Second Example" <|
                \_ -> parse example2 |> run Nothing [] 0 |> first |> equal (Array.fromList [ 2, 0, 0, 0, 99 ])
            , test "Third Example" <|
                \_ -> parse example3 |> run Nothing [] 0 |> first |> equal (Array.fromList [ 2, 3, 0, 6, 99 ])
            , test "Forth Example" <|
                \_ -> parse example4 |> run Nothing [] 0 |> first |> equal (Array.fromList [ 2, 4, 4, 5, 99, 9801 ])
            , test "Fifth Example" <|
                \_ -> parse example5 |> run Nothing [] 0 |> first |> equal (Array.fromList [ 30, 1, 1, 4, 2, 5, 6, 0, 99 ])
            ]
