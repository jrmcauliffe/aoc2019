...
suite =
    describe "Day3 Tests"
        [ describe "Path Parser"
            [ test "Up command" <|
                \_ -> "U293" |> parse |> equal (Just ( Up, 293 ))
            , test "Down command" <|
                \_ -> "D219" |> parse |> equal (Just ( Down, 219 ))
            , test "Left command" <|
                \_ -> "L3" |> parse |> equal (Just ( Left, 3 ))
            , test "Right command" <|
                \_ -> "R111" |> parse |> equal (Just ( Right, 111 ))
            , test "Missing Direction" <|
                \_ -> "111" |> parse |> equal Nothing
            , test "Invalid Direction" <|
                \_ -> "Q111" |> parse |> equal Nothing
            , test "Mising Distance" <|
                \_ -> "U" |> parse |> equal Nothing
            , test "Invalid Distance" <|
                \_ -> "UXX" |> parse |> equal Nothing
            ]
        , describe "To Line function"
            [ test "Up command" <|
                \_ -> ( Up, 1 ) |> toLine ( 0, 0 ) |> equal ( ( 0, 0 ), ( 0, 1 ) )
            , test "Down command" <|
                \_ -> ( Down, 1 ) |> toLine ( 0, 0 ) |> equal ( ( 0, 0 ), ( 0, -1 ) )
            , test "Left command" <|
                \_ -> ( Left, 1 ) |> toLine ( 0, 0 ) |> equal ( ( 0, 0 ), ( -1, 0 ) )
            , test "Right command" <|
                \_ -> ( Right, 1 ) |> toLine ( 0, 0 ) |> equal ( ( 0, 0 ), ( 1, 0 ) )
            ]
        , describe "Detect Crossing"
            [ test "Simple" <|
                \_ -> detectCrossing ( ( 0, 0 ), ( 10, 0 ) ) ( ( 5, -5 ), ( 5, 5 ) ) |> equal (Just ( 5, 0 ))
            , test "Simple 2" <|
                \_ -> detectCrossing ( ( 3, 5 ), ( 3, 2 ) ) ( ( 6, 3 ), ( 2, 3 ) ) |> equal (Just ( 3, 3 ))
            , test "Simple Fail" <|
                \_ -> detectCrossing ( ( 0, 0 ), ( 10, 0 ) ) ( ( 0, 10 ), ( 10, 10 ) ) |> equal Nothing
            , test "Simple Fail 2" <|
                \_ -> detectCrossing ( ( 5, 5 ), ( 5, 0 ) ) ( ( 0, 0 ), ( 3, 0 ) ) |> equal Nothing
            , test "Simple Fail 3" <|
                \_ -> detectCrossing ( ( 3, -1 ), ( 3, 3 ) ) ( ( 0, 0 ), ( 3, 0 ) ) |> equal Nothing
            ]
